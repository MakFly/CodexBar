...
extension SettingsStore {
    ...
    static let codexAutoFailoverDefaultThresholdPercent = 10
    static let codexAutoFailoverThresholdChoices = [5, 10, 20, 30]

    var codexAutoFailoverEnabled: Bool {
        get { self.providerConfig(for: .codex)?.codexAutoFailoverEnabled ?? false }
        set {
            self.updateProviderConfig(provider: .codex, affectsBackgroundWork: true) { entry in
                entry.codexAutoFailoverEnabled = newValue
            }
            self.logProviderModeChange(provider: .codex, field: "codexAutoFailoverEnabled", value: String(newValue))
        }
    }

    var codexAutoFailoverThresholdPercent: Int {
        get {
            Self.sanitizedCodexAutoFailoverThreshold(
                self.providerConfig(for: .codex)?.codexAutoFailoverThresholdPercent)
        }
        set {
            let sanitized = Self.sanitizedCodexAutoFailoverThreshold(newValue)
            self.updateProviderConfig(provider: .codex) { entry in
                entry.codexAutoFailoverThresholdPercent = sanitized
            }
            self.logProviderModeChange(
                provider: .codex,
                field: "codexAutoFailoverThresholdPercent",
                value: String(sanitized))
        }
    }

    static func sanitizedCodexAutoFailoverThreshold(_ value: Int?) -> Int {
        guard let value else { return self.codexAutoFailoverDefaultThresholdPercent }

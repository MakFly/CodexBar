...
struct CodexAccountsSectionView: View {
        ...
        Section {
            if let selection = self.activeSelectionBinding {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(
                        L(
                            "Chooses which Codex account CodexBar displays. " +
                                "The Codex CLI and app keep using the System account below."))
                        .font(.footnote)
    ...
    private func systemRow(selection: Binding<String>?) -> some View {
        ...
        Text(
            L(
                "The account the Codex CLI and app use on this Mac. " +
                    "Switching it swaps the shared Codex credentials; session history is kept."))
            .font(.footnote)
            .foregroundStyle(.secondary)
    }
}

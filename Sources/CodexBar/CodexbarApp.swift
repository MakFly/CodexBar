...
struct CodexBarApp: App {
    ...
    @State private var codexAccountAutoFailoverCoordinator: CodexAccountAutoFailoverCoordinator
    @State private var codexAccountSwitchIslandController: CodexAccountSwitchIslandController
    ...
    init() {
        ...
        let codexAccountAutoFailoverCoordinator = CodexAccountAutoFailoverCoordinator(
            settings: settings,
            usageStore: store,
            promotionCoordinator: codexAccountPromotionCoordinator)
        codexAccountAutoFailoverCoordinator.activate()
        let codexAccountSwitchIslandController = CodexAccountSwitchIslandController()
        codexAccountPromotionCoordinator.displayNameResolver = { [weak settings] managedAccountID in
            settings?.codexVisibleAccountProjection.visibleAccounts
                .first { $0.storedAccountID == managedAccountID }?
                .displayName
        }
        codexAccountPromotionCoordinator.onSwitchEvent = { event in
            switch event.kind {
            case .began:
                codexAccountSwitchIslandController.begin(
                    target: event.targetDisplayName,
                    trigger: event.trigger)
            case .succeeded:
                codexAccountSwitchIslandController.resolve(
                    succeeded: event.targetDisplayName,
                    trigger: event.trigger)
            case let .failed(message):
                codexAccountSwitchIslandController.resolve(failed: message, trigger: event.trigger)
            }
        }
        self.preferencesSelection = preferencesSelection
        _codexAccountAutoFailoverCoordinator = State(wrappedValue: codexAccountAutoFailoverCoordinator)
        _codexAccountSwitchIslandController = State(wrappedValue: codexAccountSwitchIslandController)

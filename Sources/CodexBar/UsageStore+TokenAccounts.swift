...
extension UsageStore {
    ...
    func shouldFetchAllCodexVisibleAccounts() -> Bool {
        ...
        // Auto failover needs every account's headroom, so it forces the multi-account fetch in any layout.
        return (self.settings.multiAccountMenuLayout == .stacked || self.settings.codexAutoFailoverEnabled) &&
    ...
    func refreshCodexVisibleAccountsForMenu(generation: UInt64? = nil) async {
        ...
        let accounts = self.limitedCodexVisibleAccounts(
            ...
            // Auto failover reads the system account's headroom, so it must survive the snapshot cap.
            pinnedVisibleAccountID: self.settings.codexAutoFailoverEnabled
                ? projection.liveVisibleAccountID
                : nil)
        ...
        defer { self.onCodexAccountSnapshotsDidRefresh?() }
    ...
    func limitedCodexVisibleAccounts(
        ...
        pinnedVisibleAccountID: String? = nil) -> [CodexVisibleAccount]
    {
        ...
        var limited = Array(accounts.prefix(limit))
        let requiredIDs = [activeVisibleAccountID, pinnedVisibleAccountID].compactMap(\.self)
        for requiredID in requiredIDs where !limited.contains(where: { $0.id == requiredID }) {
            guard let required = accounts.first(where: { $0.id == requiredID }) else { continue }
            // Drop the lowest-ranked entry that is not itself required, so pinning one never evicts the other.
            let dropIndex = limited.lastIndex { !requiredIDs.contains($0.id) } ?? limited
                .index(before: limited.endIndex)
            limited.remove(at: dropIndex)

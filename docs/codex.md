...
### Automatic account failover (opt-in)
- Settings → Providers → Codex → **Automatically switch the system account** (visible once at least two Codex
  accounts are known). Off by default. When enabled, the row below it names the account currently in the System
  slot, so it is always clear which one would be replaced.
- What it acts on: the **System account** — the credentials in the ambient `CODEX_HOME/auth.json` that the Codex
  CLI and app read. It is *not* driven by the menu switcher: selecting a row there only changes what CodexBar
  displays (`codexActiveSource`) and can neither trigger nor suppress a swap.
- On every multi-account refresh, CodexBar reads each account's headroom — the tighter of the 5-hour and weekly
  windows. When the System account's headroom is at or below the configured threshold (5/10/20/30 %, default
  10 %), the healthiest **added (managed)** account with headroom above the threshold is promoted into the System
  slot through the same path as the manual "System Account" menu, so the displaced account is preserved as a
  managed account first.
- Guard rails: never runs while a managed-account login/removal or a promotion is in flight; at most one switch per
  10-minute cooldown; never targets profile-home or non-managed accounts, accounts that need re-authentication, or
  accounts that are also at/below the threshold. Failures are logged (`codex-auto-failover`), never shown modally.
- A user notification names both ends of the swap. Enabling the option forces the multi-account fetch in any menu
  layout, and pins the System account inside the per-refresh account cap so its headroom is always known.

### System-account switch island
- Every System-account swap — the menu's System Account submenu, the Accounts settings picker, and automatic
  failover — shows a small click-through island under the notch (below the menu bar on displays without one).
- Phases: a spinner with "Switching to <account>…" (or "Quota reached — switching to <account>…" when automatic)
  while the swap runs, then a green check with "Codex now uses <account>", or the user-facing error on failure.
- It is deliberately loud: a saturated capsule (blue while switching, green on success, orange on failure) with
  white text.
- Placement stays out of the band notch utilities reserve. Apps in that category (Perch, and others) pin a fixed
  transparent canvas under the cutout — Perch's is 704×670pt at `.statusBar + 2`, above CodexBar's overlay — so
  anything drawn there is hidden behind their panel. The island therefore sits under the menu bar to the right of
  a 704pt reserved centre, narrowing to fit, and drops to the bottom of the screen only when the display is too
  narrow for a readable capsule beside that band.
- The panel never takes focus and ignores mouse events, mirroring the quota-warning overlay. Success clears after
  4.5 s, failures after 6 s, and the spinner is capped at 30 s so a stalled promotion cannot pin it on screen.
- A promotion that is refused before it starts (another account change in flight) shows no island; the caller's
  existing alert or settings notice reports it instead.

...
- Codex: show a click-through island while the System account is being switched, and when it lands — placed clear of the canvas notch utilities reserve under the cutout, so it is never hidden behind their panel — covering the System Account menu, the Accounts settings picker, and automatic failover.
- Codex: opt-in automatic system-account failover — when the System account's 5-hour or weekly headroom drops to a configurable threshold, CodexBar promotes the healthiest added account into the System slot (with cooldown, in-flight guards, and a confirmation notification naming both accounts). The menu switcher stays display-only, and the Accounts settings now spell out which selection the Codex CLI and app follow.
